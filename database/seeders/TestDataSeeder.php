...
class TestDataSeeder extends Seeder
{
    public function run(): void
    {
        ...
        for ($index = 1; $index <= 20; $index++) {
            $companyName = "DSSC Demo Employer {$index}";
        ...
        for ($index = 1; $index <= 120; $index++) {
            ...
            AlumniProfile::query()->updateOrCreate(
                ['user_id' => $user->id],
                [
                    'student_id' => '2026-'.str_pad((string) ($index + 1), 5, '0', STR_PAD_LEFT),
            ...
            $alumniUsers->push($user->load('alumniProfile'));
        }
        ...
        for ($index = 1; $index <= 50; $index++) {
            ...
            $title = "Demo Job Opportunity {$index}";
            $slug = Str::slug($title);
        ...
        foreach ($employedAlumni as $alumni) {
            EmploymentTracking::query()->firstOrCreate(
                [
                    'alumni_id' => $alumni->id,
                    'new_status' => 'employed',
